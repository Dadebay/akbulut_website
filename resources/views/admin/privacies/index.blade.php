    ...
    <div class="container-fluid">
        <div class="page-header py-3">
            <div class="row align-items-center">
                ...
                @if(!count($abouts))
                    <div class="col-auto">
                        <a href="{{ route('privacies.create') }}" class="btn btn-primary">
                            New about us text
                        </a>
                    </div>
                @endif
                        ...
                        <td><a class="dropdown-toggle btn btn-primary three-dots" type="button" data-toggle="dropdown"
                            ...
                            <div class="dropdown-menu dropdown-menu-right">
                                <div class="dropdown-item">
                                    ...
                                    <form method="post" action="{{ route('privacies.destroy', $about->id) }}"
                                        class="edit-delete" id="em-{{ $about->id }}">
                                        @csrf
                                        @method('delete')

                                        <a href="#" class="btn btn-danger btn-sm btn-block" title="remove"
                                            onclick="if(confirm('Вы уверены, что хотите удалить эту категорию?'))
                                                               { document.getElementById('em-{{ $about->id }}').submit() }"><i data-icon="remove"></i> Delete </a>
                                    </form>
                                </div>

                                <div class="dropdown-item">
                                    <a href="{{ route('privacies.edit', $about->id) }}"
