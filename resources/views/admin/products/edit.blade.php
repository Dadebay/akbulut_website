              ...
              <form role="form" method="post" action="{{ route('products.update', $product->id) }}" enctype="multipart/form-data"
               novalidate>
                ...
                <div class="card-body">
                            ...
                            <select class="form-control" name="category_id" id="category_id">
                                ...
                                @foreach ($children_categories as $item)
                                    <option value="{{$item->id}}" 
                                        {{$product->category->id == $item->id ? 'selected' : ''}}>{{$item->name_tk}}</option>
                                @endforeach
                            </select>
                        </div>      
                    ...
                    @foreach(LaravelLocalization::getSupportedLocales() as $localeCode => $properties)
                        <div class="form-group">
                            <label for="name-{{ $localeCode }}">{{ $properties['native'] }} general_info</label>        
                            <textarea id="generalinfo-{{$localeCode}}" 
                            class="form-control {{ $errors->has('general_info_.' . $localeCode) ? ' is-invalid' : '' }}" 
                            name="general_info_{{ $localeCode }}" 
                            required autocomplete="off">{{$product->getGeneralInfoTranslationAttr($localeCode)}}</textarea>
                    ...
                    <hr class="my-2">
                    <h6 class="text-muted">Product Detaylar</h6>

                    @foreach(LaravelLocalization::getSupportedLocales() as $localeCode => $properties)
                        <div class="form-group mt-3">
                            <label for="name-{{ $localeCode }}">{{ $properties['native'] }} detaylar</label>
                            <textarea id="bodypart-{{$localeCode}}" class="form-control {{ $errors->has('description_.' . $localeCode) ? ' is-invalid' : '' }}" name="description_{{ $localeCode }}"
                                 required autocomplete="off">{{$product->getDescTranslationAttr($localeCode)}}</textarea>
                        </div>
                    @endforeach
                    ...
                    <!-- Product Sliders -->
                    <div class="form-group">
                        <label for="images">Product Sliders:</label>        
                        <div  id="image_picker_sliders" class="row">
                            @if ($product->getMedia('product_sliders')->count() > 0)
                                
                                @foreach($product->getMedia('product_sliders') as  $photo)        
                                    ...
                                    <div class="col-md-4 col-sm-4 col-xs-6 spartan_item_wrapper" data-spartanindexrow="{{-$loop->iteration}}" style="margin-bottom : 20px; ">
                                        <div style="position: relative;">
                                            ...
                                            <input type="hidden" name="old_photos_slider[]" value={{ $photo->id }}>
...
$("#image_picker_sliders").spartanMultiImagePicker({
    fieldName     : 'product_sliders[]',
    dropFileLabel : "Drop Here",
...
class MyUploadAdapter {
    ...
    _initRequest() {
        ...
        // could be different.
        xhr.open( 'POST', '{{ route('products.upload') }}', true );
...
ClassicEditor.create( document.querySelector( '#bodypart-tk' ),{
        ...
        ClassicEditor.create( document.querySelector( '#bodypart-en' ), {
            extraPlugins: [ MyCustomUploadAdapterPlugin ],
        }).catch( error => { console.error( error ); });


ClassicEditor
        .create( document.querySelector( '#generalinfo-tk' ), {
        extraPlugins: [ MyCustomUploadAdapterPlugin ],
    }  )
        .catch( error => {                                  
            console.error( error );
        ...
        ClassicEditor
        .create( document.querySelector( '#generalinfo-ru' ), {
        extraPlugins: [ MyCustomUploadAdapterPlugin ],
    }  )
        .catch( error => {
            console.error( error );
        ...
        ClassicEditor
        .create( document.querySelector( '#generalinfo-en' ), {
        extraPlugins: [ MyCustomUploadAdapterPlugin ],
    }  )
        .catch( error => {
            console.error( error );
